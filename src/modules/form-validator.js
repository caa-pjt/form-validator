...
export class FormValidator {
  ...
  #formData(formElement) {
    ...
    if (formElement.elements.length === 0) {
      console.error(
        `No FormData inputs detected! Please ensure the form contains inputs.`
      );
    } else {
  ...
  setErrors() {
    for (const input in this.errors) {
      // Ignore this input and move on to the next
  ...
  #setHtmlError(input, error) {
    ...
    input.classList.contains("was-validated")
    ...
    if (
      input.nextElementSibling.dataset.inputName === input.getAttribute("name")
    ) {
    ...
    this.#FormValidatorDebug
      ? console.log(`Add error from inputName : ${input.getAttribute("name")}`)
      : null;
  }
    ...
    if (this.data[name] === "" || this.data[name] === undefined) {
      ...
      if (currentInput === "select") {
        this.#setError(
          name,
          "required",
          this.#errorMessages("select", { name: name })
        );
      } else {
  ...
  email(name) {
    if (
      !/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(this.data[name])
    ) {
  ...
  #setError(name, type, error) {
    this.#FormValidatorDebug
      ? console.log(`addError - name : ${name} | error : ${error}`)
      : null;
  ...
  #errorMessages(type, options) {
    ...
    if (!locales[localLowerCase]) {
      console.error(
        `Locale '${this.options.local}' not found. Defaulting to 'en'.`
      );
    ...
    if (!localeMessages[type]) {
      console.error(
        `No text found for error type '${type}' in locale '${this.options.local}'`
      );
